...
# Projectioned with stochasticity 
...
We'll use the *ctrl_catch* control object we made in Example 2 with decreasing catch.
...
Here we put together an example which includes a mix of target types (F and catch), relative, absolute and bounded targets and both types of stochasticity.
It just pulls together all the things that have already been covered in the tutorial.

The projection has F target that approaches F01 by 2015 target (as in Example 6) and a maximum allowable change in TAC.

```{r, p1}
proj_years <- 2009:2018
max_change_tac = 0.15
f_target
```

We want to run a stochastic projection so we use a combination of *propagate()* and *stf()* as before.
...
```
Set up recruitment residuals as before (see Example 7 above for details).
...
Making the control is quite complicated.
For each year of the projection we need to set: *year*, *quantity*, *val*, *min*, *max* and *rel.year*.
Remember: SSB targets are set in year-1.

```{r, p4}
f_control <- data.frame(
	   year = proj_years,
       quantity = "f",
       val = f_target,
       min = NA, max = NA, rel.year = NA)
catch_control <- data.frame(
	   year = proj_years,
       quantity = "catch",
       val = NA,
       max = 1+max_change_tac,
       min = 1-max_change_tac,
       rel.year = proj_years - 1)
big_control <- rbind(f_control, catch_control)
```

Sort the **data.frame** by year and make sure that absolute targets come before the relative ones.

```{r, sortc}
big_control <- big_control[order(big_control$year),]
big_control
```
...
```{r, mfc}
big_control <- fwdControl(big_control)
```
...
It looks OK so far but we need to add stochasticity to the target values, lie we did in Example 8.

We make a new *trgtArray* of the correct dimensions.

```{r, p6}
big_control@trgtArray <- array(NA,dim=c(length(proj_years)*2,3,niters),dimnames=list(1:(length(proj_years)*2), c("min","val","max"),iter=1:niters))
```
The maximum change in TAC is not stochastic so we just fill the the new array with the desired values.

```{r, p7}
big_control@trgtArray[seq(from = 2, to = length(proj_years)*2,by=2),"max",] <- 1+max_change_tac
big_control@trgtArray[seq(from = 2, to = length(proj_years)*2,by=2),"min",] <- 1-max_change_tac
```

The F target is stochastic. We add lognormally distributed noise to it in the new target array.

```{r, p8}
stoch_f <- f_target * rlnorm(length(proj_years) * niters, meanlog = 0, sdlog=0.3)
big_control@trgtArray[seq(from = 1, to = length(proj_years)*2,by=2),"val",] <- stoch_f
```

We can look at a single iteration (as a sanity check). The TAC change constraint is not stochastic but the SSB target is.

```{r, p9}
big_control@trgtArray[,,1]
```
Finally, stuff it into *fwd()*, along with the recruitment residuals, and go:
...
```{r, p11}
plot(window(res_fwd,start=2000, end=2018))
```
