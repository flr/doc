---
title: 'Forecasting for advice using FLash: Short term forecasts'
...
In FLR, the method for short term forecasts is called fwd(), which takes FLStock, FLSR, and fwdControl. 
...
Our ple4 stock goes up to `r range(ple4)[["maxyear"]]`, and in this example we want to make a 3 year projection, So we need to extend the stock by 3 years.

The projection will predict abundances in the future, but what  will the future stock weights, maturity, natural mortality, etc. be like? The assumptions on those will affect the outcome of the projection. Rather than using window() or trim() but that makes all future data NA, we use the stf() function (short term forecast) that has several options that allow to control the assumptions about the future stock parameters.

These assumptions specify how many years you want to average over to set future values For example, wts.nyears is the number of years over which to calculate the .wt, .spwn, mat and m slots. By default this is 3 years. This is a fairly standard assumption for STFs, i.e. the future mean weights at age will be the same as the mean of the last 3 years. Hence, What is going to happen in the next few years is probably similar to what happened in the last few years 
...
Now the stock goes up to `r range(ple4_stf)[["maxyear"]]` and the future weights are equal to the means of the last three observed years, as can be observed from the last six years of the stock weights of the ple4_stf object.
...
ggplot(sel[,ac((maxyr_stf-5):maxyr_stf)]) + geom_line(aes(x=age, y=data)) + facet_wrap(~year)
```
...
Meanwhile, the landings and discards are forecasted ratios of what is discarded and what is landed.  
...
A short term forecast does not use a SRR, instead it generally assumes that recruitment in the future is the mean of the historic recruitments. However, we still need to pass the projection a SRR that contains the mean value, which what we mimic for this example. First we estimate the geometic mean recruitment, that we then add to an SRR object  
...
Below we define the last year of the stock and the status quo fbar.
```{r, eval=TRUE}
...
Similarly, we check if the fbar in the forecasted stock indeed corresponds to the status quo fishing mortality. Also, we 

```{r, eval=TRUE}
fbar_SQ
fbar(ple4_sq)[,ac((maxyr_stf-5):maxyr_stf)]
...
```{r, eval=TRUE}
harvest(ple4_stf)[,ac((maxyr_stf-2):maxyr_stf)]
harvest(ple4_sq)[,ac((maxyr_stf-2):maxyr_stf)]

harvest(ple4_stf)[,ac((maxyr_stf-2):maxyr_stf)] / harvest(ple4_sq)[,ac((maxyr_stf-2):maxyr_stf)]
```

The catch numbers come from the predicted abundance and harvest rates using the Baranov equation, and that the catches are then split into landings and discards using the ratios that stf() gave us
...
Another scenario we are interested in is F0.1. We can calculate this using FLBRP (or maybe you already have a value).
...
for (scenario in 1:nrow(fbar_scenarios)) {
    ...
    # Change in SSB
    stf_results[scenario,7] <- round((ssb(ple4_fwd)[,ac(maxyr_stk+3)]-ssb(ple4_fwd)[,ac(maxyr_stk+2)])/ssb(ple4_fwd)[,ac(maxyr_stk+2)]*100,1) # change in ssb in last two stf years
    stf_results[scenario,8] <- round((catch(ple4_fwd)[,ac(maxyr_stk+2)]-catch(ple4_fwd)[,ac(maxyr_stk)])/catch(ple4_fwd)[,ac(maxyr_stk)]*100,1) # change in catch from true year, to 2nd to last stf year
}
