---
title: "Forecasting for advice using **FLasher**: Medium term forecasts"
...
ctrl_rel_catch <- fwdControl(
	data.frame(year = 2009:2018,
		   ...
		   relYear = 2008:2017))
```
NOTE: NEED TO SET relSeason TO 1 IN fwd() AND OTHERS
QUICK HACK HERE

```{r, relseason_hack}
ctrl_rel_catch@target$relSeason <- 1
ctrl_rel_catch@target$relCatch <- 1
ctrl_rel_catch@target$relFishery <- 1
```
...
Note that the data.frame is not in chronological order.
...
Note the use of the *relYear*, *min* and *max* columns in the catch data.frame.
...
```{r, relseason_hack2}
rel_rows <- !is.na(ctrl_rel_min_max_catch@target$relYear)
ctrl_rel_min_max_catch@target[rel_rows, "relSeason"] <- 1
ctrl_rel_min_max_catch@target[rel_rows, "relCatch"] <- 1
ctrl_rel_min_max_catch@target[rel_rows, "relFishery"] <- 1
```
...
RESIDUALS NEED TO BE EXPANDED IN FWD TO MATCH SAME DIM AS BIOLS
