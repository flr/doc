...
The **FLFishery** objects can be accessed from the **FLFisheries** object by the _[[_ operator (like accessing a list), either by name or position.
Each **FLCatch** within an **FLFishery** is also accessed by the _[[_ operator by name or position.
The positions are important when it comes to setting up the control object (more on this later).
...
bt1 <- FLFishery(pleBT=flfs[["bt"]][["pleBT"]])
...
Here, we have a single fishery, with a single catch fishing on a single biol.
...
In this example, the catch target relates to the total catch from that **FLBiol**. We therefore use the _biol_ column to specify this. This column takes an integer which relates to the position of the chosen **FLBiol** in the **FLBiols** list.
In this case we only have one **FLBiol** in the **FLBiols** object so the entry in the _biol_ column is 1.
...
length(biols)
# The plaice FLBiol is first in the list
names(biols)
```
...
In this first example we set only a catch target for the plaice stock. We again use the _biol_ column to specify which **FLBiol** the catch target relates to. There are two **FLBiol** objects in the **FLBiols** object. We saw above that the first one is the plaice stock and the second one is the sole stock. This means that to apply the catch target to the plaice stock we put _1_ in the _biol_ column. If we wanted to apply the catch target to the sole stock we would put _2_.
...
In this control object we set a catch target for _biol_ 1 (plaice) and also an F target for _biol_ 2 (sole) in the _biol_ column.
...
# The plaice target bit - note the NAs for minAge and maxAge
...
One option is to set target catches for each of the fisheries, i.e. a target catch for sole caught by the beam trawl and a target catch for sole caught by the gillnet. i.e. to set partial catches at the **FLCatch** level.
...
To do this we use the _fishery_ and _catch_ column to specify the position of the objects that relate to the target.
Here there is a catch target that relates to the first (only) **FLCatch** of the first **FLFishery** and another catch target that relates to the first (only) **FLCatch** of the second **FLFishery**.
...
An alternative approach is to set a total catch at the stock level (for example, if the stock was managed through a TAC) and set _relative_ catches between the fisheries (for example, by maintaining the historic relative stability of the fleets).
...
Here we are saying the catch of the first (only) **FLCatch** of the first **FLFishery** in the **FLFisheries** object is relative to the first (only) **FLCatch** of the second **FLFishery** object.
...
(All the columns with NA in them are only there to allow the _rbind()_ of the data.frames and looks a bit ugly - sorry).

```{r, 2f1s11}
sole_catch_target <- 12000
sole_bt_gn_catch_relative <- 2
flasher_ctrl_target1 <- data.frame(year = 2:20,
                        quant = "catch", relYear = NA,
                        fishery = NA, relFishery=NA,
                        catch = NA, relCatch=NA,
                        biol = 1,
                        value = sole_catch_target)
flasher_ctrl_target2 <- data.frame(year = 2:20,
                        quant = "catch", relYear = 2:20,
...
In this example we set a total sole catch (_biol = 2_ in the control object).
We also set a relative plaice catch between the fisheries. The beam trawl fishery (the first **FLFishery**, hence _fishery = 1_) takes 1.5 times as much plaice as the gillnet fishery (the second **FLFishery**, hence _relFishery_ = 2).
The plaice is caught by the first **FLCatch** of each **FLFishery** hence the use of _catch = 1_ and _relCatch = 1_.
