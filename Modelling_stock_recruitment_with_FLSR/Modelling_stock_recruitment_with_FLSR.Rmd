...
The __FLSR__ is a [S4](https://stat.ethz.ch/R-manual/R-devel/library/methods/html/Classes_Details.html) class for Stock-Recruitment (SR) models, an extension of __FLModel__ , part of the __FLCore__ package.  Commonly used or custom-tailored SR models can be fitted directly on __FLStock__ objects and provide estimates of uncertainty. __FLSR__ class objects can be then used to visualize the fitted models as well as for making future projections. 
...
The default recruitment age in FLSR is age=1. However, the user can change this by triming the __FLStock__ object while converting it into an __FLSR__ object:
...
Finally, custom SR models can be implemented. To define a new model requires the specification of it´s i) functional form, ii) likelihood iii) bounds and iv) starting values. For example below the user can fit the Deriso-Schnute model.
