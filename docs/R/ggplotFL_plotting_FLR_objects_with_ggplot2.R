...
## ---- pflqs, echo=TRUE, fig.cap="Standard ggplot2-based plot for an FLQuants object with multiple iterations, and consisting of four elements."----
fqs <- FLQuants(F = rlnorm(200, fbar(ple4), 0.15), SSB = ssb(ple4), Rec = rec(ple4), Catch = catch(ple4))
plot(fqs)

## ---- pflqsg, echo=TRUE, fig.cap="Wrap-based ggplot2-based plot for an FLQuants object with multiple iterations, and consisting of four elements."----
fqs <- FLQuants(F = rlnorm(200, fbar(ple4), 0.15), SSB = ssb(ple4), Rec = rec(ple4), Catch = catch(ple4))
plot(fqs) + facet_wrap(~qname, scales="free")

## ---- pfls,  fig.cap="A ggplot2 version of the standard plot() for FLStock, as applied to `ple4`"----
plot(ple4)
...
## ---- exsim4, echo=TRUE, cache=TRUE--------------------------------------
fdw <- reshape(fdf, timevar = "iter", idvar = c("year"), direction = "wide")

## ---- exsim5, echo=TRUE, cache=TRUE--------------------------------------
levels(fdf[,'iter'])

## ---- exsim6, echo=TRUE, cache=TRUE, fig.cap="Time series with 75% and 90% credibility intervals plotted using geom_ribbon."----
...
## ---- exspa, echo=TRUE, cache=TRUE, fig.cap="Spaghetti plot of a stochastic simulation, by calling geom_line on top of the stored ribbon plot."----
