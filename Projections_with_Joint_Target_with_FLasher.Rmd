...
We can set an additional constraint of maximum total effort of both fisheries.
We have two efforts to solve which means that the targets in each time step (year) of the control object must come in pairs.
When we include a maximum constraint on effort we must also include an accompanying target to be solved at the same time.
As constraint targets (maximum and minimum targets) are solved after non-constraint targets, the accompanying target must also be a constraint.
We can set a constraint that is never triggered just to make up the numbers (however, it cannot be a separate minimum effort target for the two fishery objects).
