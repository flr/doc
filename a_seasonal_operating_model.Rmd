...
This vignette shows how to condition a seasonal Operating Model, i.e. one with time steps that are less than annual, based on life history theory.
...
The packge `FLife` is required for modelling life-histories and 'mydas' for estimation and developing harvest control rules

The simplest way to obtain the `FLR` pacakges aree  to install them from the `FLR` repository 
...
Conditioned an operating model on life history parameters allows more flexibility that using a data-rich stock assessment, that may never have been formally validated.

There are relationships between the life-history parameters, e.g. the von Bertalanfy growth parameters $L_{\infty}$, $k$ and $t_0$, length at maturity ($l_{50}$), and of the length weight relationship $W=aL^b$. 

There is an example data set `teleost`  in `FLife` 
...
This means that even if you just know the maximum size of a species you can derive all the other parameters
...
Once you have the parameters, then you can create the equilibrium dynamics using the `FLBRP` class.
```{r, eq, echo=TRUE, caption="a"}
eq=lhEql(par)
...
As example, simulate a time series of fishing mortality, e.g. where an unexploited stock is fished until it is over exploited, after which a recovery plan is implemented.
...
Perform a stochastic projection
...
The procedure is as for the annual model, then take the annual `FLStock` object and *"seasonalise"* it, i.e. interpolate growth, and split natural mortality into seasons. 
```{r, om4, echo=TRUE}
...
```{r}
save(om4,eq,file="/home/laurie/Desktop/tmp/t.RData")
```

```{r, om4-3, echo=TRUE, eval=F}
om=annualise(om4)
