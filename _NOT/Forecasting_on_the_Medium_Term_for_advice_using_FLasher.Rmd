...
*THE FWD FUNCTION IS NOT WORKING IN THIS TUTORIAL*
...
We initially use `stf()`, which makes a lot of assumptions, to set up a future stock, (see the STF tutorial). We may want to change some of these assumptions, but for the moment we will use the defaults.
...
BUG IN FWD: f_status_quo NEEDS TO BE MULTIPLIED BY 10 TO GET THE DESIRED RESULT 
...
BUG IN FWD: future_catch NEEDS TO BE MULTIPLIED BY 10 TO GET THE DESIRED RESULT 
...
BUG IN FWD: DO NOT HIT future_ssb TARGET, BUT ONE TENTH OF IT
...
ctrl_rel_catch
```
BUG HERE WITH VALUE IN THE RELATIVE CALC (ORDER OF MAGNITUDE OUT, SO VALUE=0.9 DOES NOT DO AS INTENDED AND NEEDS TO BE SET TO VALUE=9)
...
ANOTHER BUG HERE WITH VALUE AND MIN - THESE NEED TO BE MULTIPLIED BY 20 TO GIVE THE DESIRED RESULT!!!
...
BUG IN FWD: DON'T KNOW WHAT THIS IS DOING, BUT IT IS NOT THE DESIRED RESULT.
...
This takes a little time (we have 200 iterations).
...
This is currently length 1. We have 200 iterations and therefore we need to expand *iters* along the iter dimension so it can store the 200 iterations.
