...
```{r}
ak  =mydas:::invAlk(par,cv=0.1)  
lfd=mydas:::lenSample(catch.n(om)[,ac(60:120)],ak,nsample=500)
```

```{r}
save(lfd,file="/home/laurie/Desktop/tmp/lfd.RData")
```

```{r, fig-lfd, fig.width=10,fig.height=10} 
library(ggpubr)  

load("/home/laurie/Desktop/tmp/lfd.RData")
...
```{r}
save(lbi,om,eql,file="/home/laurie/Desktop/tmp/tmp.RData")
```
...
load("/home/laurie/Desktop/tmp/tmp.RData") 
...
transform(rtn[abs(rtn$score-1)==min(abs(rtn$score-1)),],roc.TSS=TPR-FPR) 
```

```{r}
ggplot(rtn)+
  geom_line(aes(FPR,TPR))+
  geom_point(aes(FPR,TPR),data=rtn[abs(rtn$score-1)==min(abs(rtn$score-1)),],col="red",size=3)
```
