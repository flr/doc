...
install.packages("FLife", repos = "http://flrproject.org/R")
```
...
```{r, lenwt, eval=!FALSE}
FLCore:::lmean(samps) / (0.75 * FLCore:::lc50(samps) + 0.25 * 60) #
lenwt <- FLPar(a=0.01030, b=2.975)
```


```{r, lmaxy, eval=!FALSE}
FLCore:::lmaxy(samps, lenwt)
```


```{r, pmega, eval=!FALSE}
FLCore:::pmega(samps, linf=60)
```
