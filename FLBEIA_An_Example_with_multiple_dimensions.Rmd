...
SM_bio    <- bioSum(SM, long = TRUE)             # Data frame (DF) with the biological indicators.
SM_adv    <- advSum(SM, long = TRUE)             # DF with the indicators related with the management advice (TAC). 
SM_flt    <- fltSum(SM, long = TRUE)             # DF with the indicators at fleet level.
SM_fltStk <- fltStkSum(SM, long = TRUE)          # DF with the indicators at fleet and stock level.
SM_mt     <- mtSum(SM, long = TRUE)              # DF with the indicators at fleet.
SM_mtStk  <- mtStkSum(SM, long = TRUE)           # DF with the indicators at fleet and metier level.
SM_vessel <- vesselSum(SM, long = TRUE)          # DF with the indicators at vessel level.
SM_vesselStk <- vesselStkSum(SM, long = TRUE)    # DF with the indicators at vessel and stock level.
...
plot(SM$stocks[[1]])  # The stock 2 is in biomass, the function 'plot(SM$stocks[[2]])' does not work.
```
...
Now change the price dynamics defining ```elasticPrice``` model in one fleet ```fl1``` and stock ```stk2```.  For a detailed description of this function see page 19 of the  [Manual](https://github.com/flr/FLBEIA/blob/master/inst/doc/FLBEIA_manual.pdf ).
...
RefPts$value <- c(c(800, 0.11, 800, 550, 0.25, 0.50), c(800, 0.2, 800, 550, 0.25, 0.50), c(800, 0.2, 800, 550, 0.25, 0.50), 
                  c(800, 0.2, 800, 550, 0.25, 0.50), c(800, 0.2, 800, 550, 0.25, 0.50), c(800, 0.2, 800, 550, 0.25, 0.50))
