---
title: "A simple example with multiple dimensions in FLBEIA "
...
plot(SM$stocks[[1]])
#plot(SM$stocks[[2]]) # Error: Recently FLCore has changed its code, FLBEIA will update its code to solve this issue.
```
...
#subset(SM_x, indicator == 'price' & fleet == 'fl1' & stock == 'stk2')
SM_x <-  subset(SM_x, value > 0) # Error en el indicador, no s? porqu? sale
...
### Variable Costs
Variable costs decreases a 50% for ```fl2``` and  for metiers ```met1``` and ```met2```. How affects that to the economic results?
```{r echo=TRUE, , results = "hide"}
multiFl_3 <- multiFl
...
Now, imagine that the crew share remuneration system changes. Now the crew share is 20% - 25%. Imaging that the system has changed for ``fl1```` and now the percentage is 50%. 
```{r echo=TRUE, results = "hide"}
multiFl_4 <- multiFl
...
[more explanatioin about capital dynamics]
...
There are no  variation between scenarios because in SCD investment in new vessels will only occur if the operational days of existing vessels is equal to maximum days.
```{r echo=TRUE, eval=TRUE}
SM_5$fleets$fl2@effort
SM_5$fleets$fl2@capacity
