...
- FLR: [FLCore](http://www.flrproject.org/FLCore/), [FLash](http://www.flrproject.org/FLCore/),[FLXSA](http://www.flrproject.org/FLCore/),[FLBRP](http://www.flrproject.org/FLCore/), [ggplotFL](http://www.flrproject.org/ggplotFL/)

You can do so as follows,

```{r, eval=FALSE}
install.packages(c("ggplot2"))
install.packages(c("mse","FLa4a","FLasher","FLXSA","FLBRP","ggplotFL"), repos="http://flrproject.org/R")
```
...
#
srr <- window(srbh, end=fy)
residuals(srr)[, ac(iy:fy)] <- exp(srbh.res)
```
...
SBmsy <- c(refpts(brp)["msy","ssb"])
SBpa <- 0.5*SBmsy
SBlim <- SBpa/1.4
rpts <- FLPar(FMSY=Fmsy, MSY=msy, SBMSY=SBmsy, SBpa=SBpa, SBlim=SBlim)
# Prepare the FLStock object for projections
stk <- stf(stk, fy-dy, nsqy, nsqy)
```

```{r om}
om <- FLom(stock=stk, sr=srr, refpts=rpts, projection=mseCtrl(method=fwd.om))
```
...
oem <- FLoem(deviances=list(
  stk=FLQuants(catch.n=rlnorm(it, catch.n(stk) %=% 0, 0.1))),
  method=sampling.oem,
  observations=list(stk=stk, idx=idx))

mseargs <- list(iy=2017, fy=2029)

control <- mpCtrl(list(
  hcr = mseCtrl(method=fixedF.hcr, args=list(ftrg=0.15)),
  est = mseCtrl(method=perfect.sa)))

perfxf <- mp(om, oem=oem, ctrl=control, args=mseargs)

control <- mpCtrl(list(
  hcr = mseCtrl(method=fixedF.hcr, args=list(ftrg=0.15)),
  est = mseCtrl(method=xsa.sa)))

xsafxf <- mp(om, ctrl=control, args=mseargs)

control <- mpCtrl(list(
  hcr = mseCtrl(method=ices.hcr, args=list(ftrg=0.15, sblim=100000,
    sbsafe=300000)),
  est = mseCtrl(method=xsa.sa)))

xsaices <- mp(om, ctrl=control, args=mseargs)

plot(om, list(PFX=perfxf, XFX=xsafxf, XIC=xsaices))

data(statistics)

per <- performance(list(perfxf, xsafxf, xsaices),
  statistics=statistics[c('SBMSY', 'FMSY', 'risk1', 'risk2', 'risk3', 'PBlim', 'C')],
  metrics=list(C=catch, F=fbar, SB=ssb),
  years=list(2018:2025), mp="test")

library(mseviz)

plotBPs(per)

plotTOs(per, 'C', c('SBMSY', 'FMSY', 'PBlim'))
