...
Alternatively, an SR model can be provided (see the tutorial on [modelling stock recruitment with FLSR](http://doc.flr-project.org/Modelling_stock_recruitment_with_FLSR.html)). For example, a Ricker stock-recruitment relationship for the `ple4` stock object could be specified. The `FLSR` object is first created and then fitted, after re-scaling the input values to help the optimizer. The parameter values are then scaled back and used to construct an `FLBRP` object where the Ricker model is to be used in the calculations.
